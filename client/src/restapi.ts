...
export class RestAPI {
  ...
  public verify(verificationCode: string, type: IdentityType) {
    ...
    if (accountId === null) {
      throw new Error("Unknown AccountID");
    }
    switch (type) {
      case IdentityType.Phone:
        return this.accountCall(
          "POST",
          "v1/verify/phonenumber",
          JSON.stringify({ accountId, verificationCode })
        );
  ...
  public async verifyWithToken(
    verificationToken: string,
    verificationCode: string
  ) {
    const response = await this.accountCall(
      "POST",
      "v2/signup/confirm",
      JSON.stringify({ verificationToken, verificationCode })
    );
  ...
  public async verifyCheck(type: IdentityType) {
    ...
    const params = new URLSearchParams({ accountId: accountId! }).toString();
    let url: string;
    switch (type) {
      case IdentityType.Phone:
  ...
  public async refreshSession() {
    ...
    if (
      ...
      console.log(
        "It seems that the refresh token has expired, attempting to refresh session regardless."
      );
    }
    const response = await this.authCall(
      "v1/token",
  ...
  public async reportPaymentNonce(productCode: string, nonce: string) {
    const response = await this.paymentCall(
      "POST",
      "v1/braintree/payment",
      JSON.stringify({ nonce, productCode })
    );
    const body = await response.json();
    if (typeof body === "object") {
  ...
  public async reportPaypalOrder(orderId: string, productCode: string) {
    const response = await this.paymentCall(
      "POST",
      "v1/paypal/payment",
      JSON.stringify({ orderId, productCode })
    );
    const body = await response.json();
    if (typeof body === "object") {
  ...
  public async getPassList() {
    ...
    if (passIds.length > 0) {
      ...
      const response = await this.productCall(
        "GET",
  ...
  public async redeemTicket(ticketId: string, region: string) {
    const response = await this.organizationCall(
      "POST",
  ...
  public async redeemEventTicket(ticketId: string, region: string) {
    const response = await this.organizationCall(
      "POST",
      "v1/eventTicket/" + ticketId + "/activate",
      region
    );
    const body = await response.json();
    return body;
  ...
  private organizationCall(
    ...
    return this.call(
      method,
      getOrganizationEndpoint(region),
      route,
      body,
      refresh
    );
  ...
  private async fetchRoute(
    ...
    if (this.store.getState().account.unstableConnection) {
      ...
      const delay =
        minDelay + Math.pow(Math.random(), delaySkew) * (maxDelay - minDelay);
      console.log(
        `Delaying request to '${route}' by ${Math.round(
          delay
        ).toLocaleString()}ms`
      );
      await new Promise((resolve, reject) => {
        if (Math.random() <= failureRate) {
          ...
          setTimeout(
            () => reject(new RestAPIError(RestAPIErrorType.MOCK, {})),
            delay
          );
        } else {
    ...
    if (typeof state.account.accessToken === "string") {
      ...
      if (
        typeof state.account.accessTokenExpire === "number" &&
        state.account.accessTokenExpire < Date.now()
      ) {
        console.log(
          "It seems like the access token has expired, attempting request regardless"
        );
      }
    }
